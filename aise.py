...
class GeneticOperator:
    ...
    def __init__(self, mut_range, mut_prob, cliprg, op_type='mutate'):
    ...
    def __call__(self, *args):
        if self.op_type == "mutate":
            base, *_ = args
            return self.mutate(base, self.cliprg)
        elif self.op_type == "crossover":
            assert len(args) == 2
...
class AISE:
    ...
    def __init__(
        ...
        if input_shape is None:
            try:
                ...
                logging.warning("Invalid data type for x_orig!")
        else:
        ...
        if query_objects is None:
            try:
                ...
                logging.warning("Cannot build query objects!")
        else:
            self._query_objects = [
                AnnoyIndex(f=query_objects["f"],metric=query_objects["metric"])
                for _ in range(self.n_class)
            ]
            for q,fp in zip(self._query_objects,query_objects["paths"]):
                q.load(os.path.join(query_objects["saved_object_dir"],fp))
    ...
    def _build_class_query_object(self, xh_orig, class_label=-1):
        ...
        query_object.save(f"./query_objects/class{class_label}l{self.hidden_layer+1}.ann")
    ...
    def _build_all_query_objects(self):
        ...
        if self.n_class:
            logging.info("Building query objects for {} classes {} samples...".format(self.n_class, self.x_orig.size(0)))
            query_objects = [self._build_class_query_object(xh_orig, class_label=i) for i in range(self.n_class)]
        else:
            logging.info("Building one single query object {} samples...".format(self.x_orig.size(0)))
...
#                 breakpoint()
            ...
            logging.info("Searching {} naive B cells for each of {} antigens...".format(self.n_neighbors, Q.size(0)))
    ...
    def _hidden_repr_mapping(self, x, batch_size=1000, query=False):
        """
    ...
    def generate_b_cells(self, ant, ant_tran, nbc_ind, y_ant=None):
        ...
        if self.keep_memory:
            ...
            mem_bcs = None
            mem_labs = None
        logging.info("Affinity maturation process starts with population of {}...".format(self.n_population))
        if self.return_log:
        ...
        for n in range(ant.size(0)):
            genop = GeneticOperator(self.mut_range[1], self.mut_prob[1], self.cliprg, op_type=self.genop_type)
...
#                         true_class_fitness = fitness_score[y_ant[n]].sum().item()
    ...
    def clonal_expansion(self, ant, y_ant=None):
        logging.info("Clonal expansion starts...")
        ...
        if self.keep_memory:
            logging.info("{} plasma B cells and {} memory generated!".format(pla_labs.shape[0] * self.n_plasma,
                                                                            mem_labs.shape[0] * self.n_memory))
        else:
            logging.info("{} plasma B cells generated!".format(pla_labs.shape[0] * self.n_plasma))
        return mem_bcs, mem_labs, pla_bcs, pla_labs, ant_logs
